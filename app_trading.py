...
with tab_journal:
    ...
    with st.expander("📜 VOIR L'HISTORIQUE DÉTAILLÉ", expanded=False):
        if history:
            ...
            st.dataframe(df_hist[cols_valides].iloc[::-1], use_container_width=True)        "SORTIE": f"{exit_price:.4f}"
    })
    save_data(HIST_FILE, history)
    return pnl
...
# Utilisation de KuCoin pour éviter les blocages IP sur Streamlit Cloud
exchange = ccxt.kucoin({'timeout': 30000, 'enableRateLimit': True})
...
def get_market_analysis(symbol, mode_choisi):
    try:
        ...
        # Calcul ADX (Force de la tendance)
        ...
        return {
            "symbol": symbol, "prix": prix, "adx": adx, "atr": atr,
            "upper": bb['BBU_20_2.0'].iloc[-1], "lower": bb['BBL_20_2.0'].iloc[-1]
        }
    except: return None
...
try:
    test_p = exchange.fetch_ticker('BTC/USDT')['last']
    st.success(f"✅ CONNECTÉ AU CLOUD | BTC: {test_p}$")
except:
    st.error("❌ ERREUR API (Vérifiez la connexion)")

tab_scan, tab_search, tab_journal = st.tabs(["🔎 SCANNER DYNAMIQUE", "🔍 RECHERCHE PAIRE", "📈 JOURNAL"])
...
# --- ONGLET 1 : SCANNER (TRI DYNAMIQUE) ---
with tab_scan:
    col_list, col_focus = st.columns([1, 2])
    with col_list:
        if st.button("🚀 LANCER SCAN"):
            with st.status("Analyse du marché en cours..."):
                tickers = exchange.fetch_tickers()
                valid_pairs = []
                for s, t in tickers.items():
                    if s.endswith('/USDT'):
                        v = t.get('quoteVolume') or t.get('info', {}).get('volValue', 0)
                        if v and float(v) > 50000: # On filtre les paires trop mortes
                            valid_pairs.append({'s': s, 'v': float(v)})
                
                # Analyse du Top 40 par volume
                top_40 = sorted(valid_pairs, key=lambda x: x['v'], reverse=True)[:40]
                
                analyzed_list = []
                for p_item in top_40:
                    ana = get_market_analysis(p_item['s'], mode_actuel)
                    if ana: analyzed_list.append(ana)
                
                # TRI DYNAMIQUE : Au lieu de rien afficher, on trie par les meilleurs ADX
                if "RANGE" in mode_actuel:
                    # Plus l'ADX est bas, plus c'est un bon range
                    analyzed_list.sort(key=lambda x: x['adx'])
                else:
                    # Plus l'ADX est haut, plus la tendance est forte
                    analyzed_list.sort(key=lambda x: x['adx'], reverse=True)
                
                st.session_state['scan_res'] = [x['symbol'] for x in analyzed_list[:15]]
    ...
    with col_focus:
        if 'active_p' in st.session_state:
            ...
            if res:
                st.header(f"💼 Analyse : {res['symbol']}")
                st.write(f"**Indicateur ADX :** {res['adx']:.1f}")
                ...
                if st.button("🚀 VALIDER ET SURVEILLER"):
                    ...
                    save_data(DB_FILE, st.session_state['test_positions'])
                    st.success("Position ajoutée au journal !")

# --- ONGLET 2 : RECHERCHE ---
with tab_search:
    pair_input = st.text_input("Symbole (ex: SOL, PEPE, BTC)").upper().strip()
    if pair_input:
        ana = get_market_analysis(pair_input, mode_actuel)
        if ana:
            st.metric("PRIX LIVE", f"{ana['prix']}", f"ADX: {ana['adx']:.1f}")
    ...
    if st.session_state['test_positions']:
        for p, d in list(st.session_state['test_positions'].items()):
            try:
                ...
                with st.expander(f"📊 {p} | {pnl:+.2f}%", expanded=True):
                    st.write(f"Entrée: {d['entry']} | Prix actuel: {curr}")
