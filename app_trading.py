        ...
        if st.button(f"🚀 LANCER SCAN"):
            with st.status("Analyse Flux KuCoin..."):
                ...
                pairs = sorted([s for s in t_s if s.endswith('/USDT')], 
                               key=lambda x: t_s[x].get('quoteVolume', 0) or 0, reverse=True)[:nb_scan]
                ...
                # Tri dynamique par Score
                results.sort(key=lambda x: x['score'], reverse=True)
                st.session_state['scan_res'] = results

        if 'scan_res' in st.session_state:
            for r in st.session_state['scan_res']:
                icon = "🟢" if r['struct'] == "BULLISH" else "🔴" if r['struct'] == "BEARISH" else "🟡"
                if st.button(f"{icon} {r['symbol']} | Score: {r['score']}"):
                    st.session_state['active_p'] = r

    with c_focus:
        if 'active_p' in st.session_state:
            res = st.session_state['active_p']
            st.header(f"💼 Analyse : {res['symbol']}")
            
            # Niveaux
            entry = res['prix']
            sl = entry - (res['atr'] * 2) if res['struct'] != "BEARISH" else entry + (res['atr'] * 2)
            tp = entry + (res['atr'] * 4) if res['struct'] != "BEARISH" else entry - (res['atr'] * 4)
            
            st_c1, st_c2, st_c3 = st.columns(3)
            st_c1.error(f"SL: {sl:.6f}")
            st_c2.warning(f"ENTRY: {entry:.6f}")
            st_c3.success(f"TP: {tp:.6f}")
            
            st.metric("ADX", f"{res['adx']:.1f}", res['struct'])
            
            if st.button("🚀 VALIDER ET SURVEILLER"):
                st.session_state['test_positions'][res['symbol']] = {
                    "entry": entry, "sl": sl, "tp1": tp, "score": res['score'],
                    "style": mode_actuel, "market_type": res['market_type'],
...
with tab_journal:
    ...
    if st.session_state['test_positions']:
        for p, data in list(st.session_state['test_positions'].items()):
            ticker = exchange_spot.fetch_ticker(p)
            cur_p = ticker['last']
            pnl = ((cur_p - data['entry']) / data['entry']) * 100
            
            with st.expander(f"📊 {p} | {pnl:+.2f}%", expanded=True):
                col1, col2 = st.columns(2)
                col1.write(f"Style: {data['style']} | Score: {data['score']}")
                if col2.button(f"Fermer {p}", key=f"close_{p}"):
